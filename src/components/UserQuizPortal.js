...
//Server GET Method ****Request Data to Database****
    ...
    return(
        <div className="pt-2">   
            ...
            {/* Score Display */}
            {score>(questions+1)/2?setResult('Well Done You are passed'):setResult('Sorry You are Failed')} 
            
            {/* Result Display */}
            {resultEnable?
            <div>
                <div className="mr-4 result-container-css">                         
                    <button className="col-12 btn text-left p-2 m-2 btn-overlay text-center result-button-css" 
            ...
            <div>
                ...
                <div className="col-12">
                    <div className="question-css">{questions+1}. {dataquestion[questions].question}</div>
                </div>
            </div>
            <div className="mr-4 option-css">
                    <button value={dataquestion[questions].option1} className="col-12 btn text-left p-2 m-2 btn-overlay quiz-button" onClick={scoreAndNext} style={{pointerEvents:buttonClick}}>{dataquestion[questions].option1}</button> 
                    <button value={dataquestion[questions].option2} className="col-12 btn text-left p-2 m-2 btn-overlay quiz-button" onClick={scoreAndNext} style={{pointerEvents:buttonClick}}>{dataquestion[questions].option2}</button>
                    <button value={dataquestion[questions].option3} className="col-12 btn text-left p-2 m-2 btn-overlay quiz-button" onClick={scoreAndNext} style={{pointerEvents:buttonClick}}>{dataquestion[questions].option3}</button>
                    <button value={dataquestion[questions].option4} className="col-12 btn text-left p-2 m-2 btn-overlay quiz-button" onClick={scoreAndNext} style={{pointerEvents:buttonClick}}>{dataquestion[questions].option4}</button>
            </div>
            </div>:''}    
            
            {/* Instant Display */}
            <div className="mr-4 option-css">                         
                <button className="col-12 btn text-left m-2 btn-overlay result-button-css" 
                        onClick={scoreAndNext}>
                    Your Answer is  
    ...
    return (
        <div className="container-fluid container-css">
            ...
            <p className="p-2 p-css">Total Questions is {dataquestion.length}</p>
            <QuestionsAnOptions/>
        </div>
    )
}
